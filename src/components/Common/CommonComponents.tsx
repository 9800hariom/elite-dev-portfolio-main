...
export const ResumeButton = ({ variant = 'primary' }: { variant?: 'primary' | 'secondary' | 'header' }) => {
    const styles = {
        ...
        secondary: "flex items-center gap-2 border border-gray-700 hover:border-cyan-500/50 text-gray-300 hover:text-cyan-400 px-8 py-3 rounded-full font-medium transition-all",
    ...
    return (
        <a href="/Hariom yadav resume.pdf" target="_blank" rel="noopener noreferrer" className={styles[variant]}>
            <Download size={variant === 'header' ? 16 : 20} />
            <span>{variant === 'header' ? 'Resume' : 'Download CV'}</span>
        </a>
    );
};
